...
---class DiningPhilosophers
    ...
    --- vid pickupForks(int id)      # Filozof próbuje podnieść widelce
    --- void eat(int id)             # Symuluje jedzenie
    --- void putdownForks(int id)    # Filozof odkłada widelce
    ---  id printState(int id, const string &state)  # Wypisuje stan filozofa
    ---void start()                  # Rozpoczyna symulację
    --- ~DiningPhilosophers()        # Destruktor (opcjonalnie)

ZMIENNE:

1. numPhilosophers	int	Liczba filozofów przy stole.
2. philosophers	vector<thread>	Tablica wątków dla każdego filozofa.
3. state	vector<int>	Przechowuje aktualny stan każdego filozofa (THINKING, HUNGRY, EATING).
4. forks	vector<bool>	Widelce – true, jeśli wolny; false, jeśli zajęty.
